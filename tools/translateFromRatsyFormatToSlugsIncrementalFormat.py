...
def performConversion(ltlFile):
    ...
    # Read inputBits
    inputBits = [a.strip() for a in stringsOfType[1].split(";") if a.strip()!=""]
    outputBits = [a.strip() for a in stringsOfType[2].split(";") if a.strip()!=""]
    ...
    buildPropertySet(assumptionProperties,"A",False,False)
    buildPropertySet(assumptionProperties,"A",False,True)
    buildPropertySet(guaranteeProperties,"G",True,False)
    buildPropertySet(guaranteeProperties,"G",True,True)
...
def buildPropertySet(properties,fileprefix,startWithNextOperatorForLivenessProperties,handleInitialisationProperties):
    ...
    if handleInitialisationProperties:
        ...
        for a in initializationProperties:
            print "AI"+fileprefix+" "+fileprefix+"I"+str(number)+" "+" ".join(a)
            number += 1
    else:
        number = 0
        for a in safetyProperties:
            print "AS"+fileprefix+" "+fileprefix+"S"+str(number)+" "+" ".join(a)
            number += 1
        for a in livenessProperties:
            print "AL"+fileprefix+" "+fileprefix+"L"+str(number)+" "+" ".join(a)
            number += 1
